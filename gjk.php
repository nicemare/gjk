...
function boba($array, $token)
{
	$req = request("/v2/customer/cards/home?filters=", "$token");
	if (preg_match('/"' . preg_quote("gojek:\/\/growth\/promo?source=demo&code=GOFOODBOBA19", '/') . '"/i', json_encode($req))) {
		echo "You got GOFOODBOBA19\n";
		$code = "GOFOODBOBA07";
	} elseif (preg_match('/"' . preg_quote("gojek:\/\/growth\/promo?source=demo&code=GOFOODBOBA07", '/') . '"/i', json_encode($req))) {
		echo "You got GOFOODBOBA07\n";
		$code = "GOFOODBOBA19";
	} else {
		echo "You are Unlucky\n";
		$code = "SadGayn";
	}
	if (($key = array_search($code, $array)) !== false) {
		unset($array[$key]);
	}
	return $array;
}
...
if ($type == 1) {
	...
	echo "\n";
	$makan = file_get_contents('20k.ktxt');
